...
const MocApproverMocRequest: React.FC<MocRequestCreationProps> = ({ onCancel }) => {
  ...
  useEffect(() => {
    const fetchMoCData = async () => {
      if (!moc_request_no) return; // guard clause
      try {
        setLoading(true);
  ...
  setCurrentStep(2);
  navigate(
    `/station-operations/Moc/NextApprover/${encodeURIComponent(moc_request_no)}`,
    { state: { step: 2 } }
  );
};
...
export default MocApproverMocRequest;